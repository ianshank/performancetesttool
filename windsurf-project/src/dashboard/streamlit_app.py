...
def main():
    ...
    st.set_page_config(
        page_title="NLM Dashboard", page_icon="📊", layout="wide", initial_sidebar_state="expanded"
    )
    ...
    with st.sidebar:
        ...
        env = st.selectbox("Environment", ["dev", "qa", "stage", "prod"], index=0)
    ...
    with col1:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(
            ["Response Times", "Throughput", "Error Rates", "System Metrics"]
        )
...
def display_error_rate_chart():
    ...
    error_rate = (
        df.groupby("minute")
        .agg({"success": lambda x: (x == False).sum() / len(x) * 100})
        .reset_index()
    )
    ...
    fig.update_layout(
        title="Error Rate Over Time", xaxis_title="Time", yaxis_title="Error Rate (%)", height=400
    )
...
def display_ai_analysis():
    ...
    summary = {
        "test_name": "NLM Test",
        "duration": (
            (df["timestamp"].max() - df["timestamp"].min()).total_seconds() if len(df) > 1 else 0
        ),
        ...
        / ((df["timestamp"].max() - df["timestamp"].min()).total_seconds() or 1),
        "min_response_time": df["response_time"].min(),
        "max_response_time": df["response_time"].max(),
        "errors": (
            df[df["success"] == False]["status_code"].value_counts().to_dict()
            if failed_requests
            else None
        ),
    }
