...
class GUIInterface(QMainWindow):
    ...
    def create_results_tab(self, tabs):
        ...
        self.summary_labels = {}
        for field in ["Test Name", "Duration", "Total Requests", "Successful", "Failed", 
                     "Avg Response Time", "Throughput"]:
            label = QLabel("--")
    ...
    def add_database_request(self):
        ...
        if dialog.result_data:
            ...
            self.logger.info(f"Added database query: {data['db_type']} {data['host']}:{data['port']}")
    ...
    def remove_selected_test_element(self):
        ...
        if current_item:
            ...
            QMessageBox.information(self, "No Selection", 
                                   "Please select a test element to remove.")
        ...
        try:
            ...
            if not test_config:
                QMessageBox.warning(self, "Configuration Error", 
                                   "Please configure at least one test target.")
                return
    ...
    def update_summary(self, results: Dict[str, Any]):
        ...
        self.summary_labels["Avg Response Time"].setText(f"{results.get('avg_response_time', 0):.3f}s")
    ...
    def show_about(self):
        """Show about dialog"""
        QMessageBox.about(self, "About NLM", 
        ...
        if column == 0:
            ...
            if data.get("type") == "http":
                ...
                if dialog.result_data:
                    ...
                    item.setText(0, f"HTTP Request - {updated_data['method']} {updated_data['url']}")
                    self.logger.info(f"Updated HTTP request: {updated_data['method']} {updated_data['url']}")
                ...
                if dialog.result_data:
                    ...
                    item.setData(0, Qt.ItemDataRole.UserRole, updated_data)
                    item.setText(0, f"Database Query - {updated_data['db_type']} {updated_data['host']}:{updated_data['port']}")
                    self.logger.info(f"Updated database query: {updated_data['db_type']} {updated_data['host']}:{updated_data['port']}")
