...
class CSVExporter:
    ...
    def export_summary_report(self, summary: Dict[str, Any], 
                             output_path: str) -> bool:
        """
    ...
    def export_ai_analysis(self, analysis: Dict[str, Any], 
                          output_path: str) -> bool:
        """
    ...
    def create_comprehensive_report(self, test_results: List[Dict[str, Any]],
    ...
    def _create_master_report(self, summary: Dict[str, Any],
            ...
            if metrics:
                for metric_name, metric_values in metrics.items():
                    if isinstance(metric_values, list) and metric_values:
                        ...
                        try:
                            avg_value = sum(float(v) for v in metric_values if isinstance(v, (int, float))) / len(metric_values)
                            master_data[f"avg_{metric_name}"] = avg_value
                        except:
                            master_data[f"metric_{metric_name}"] = str(metric_values[:5])  # First 5 values
    ...
    def export_results(self, results: List[Dict[str, Any]], summary: Dict[str, Any], 
                      filename: str = "") -> str:
        ...
        with open(filepath, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
                'timestamp', 'user_id', 'method', 'url', 'status_code', 
                'response_time', 'success', 'error', 'request_size', 'response_size'
            ]
        ...
        with open(summary_filepath, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            writer.writerow(['Average Response Time', f"{summary.get('avg_response_time', 0):.3f}s"])
            ...
            if performance_insights:
                ...
                writer.writerow(['Overall Assessment', performance_insights.get('overall_assessment', 'N/A')])
            ...
            if target_breakdown:
                ...
                writer.writerow(['Target', 'Total Requests', 'Success Rate', 'Avg Response Time (s)', 'Min Response Time (s)', 'Max Response Time (s)'])
            ...
            if user_breakdown:
                ...
                writer.writerow(['User ID', 'Total Requests', 'Success Rate', 'Avg Response Time (s)', 'Min Response Time (s)', 'Max Response Time (s)'])
            ...
            if time_series_analysis:
                ...
                if time_buckets:
                    ...
                    writer.writerow(['Timestamp', 'Requests', 'Success Rate', 'Avg Response Time (s)'])
                ...
                if trend_analysis:
                    ...
                    writer.writerow(['Metric', 'Value'])
                    writer.writerow(['Response Time Trend', trend_analysis.get('response_time_trend', 'N/A')])
                    writer.writerow(['Success Rate Trend', trend_analysis.get('success_rate_trend', 'N/A')])
                    writer.writerow(['Performance Degradation', 'Yes' if trend_analysis.get('performance_degradation') else 'No'])
                    writer.writerow(['Stability Score', f"{trend_analysis.get('stability_score', 0):.2f}"])
