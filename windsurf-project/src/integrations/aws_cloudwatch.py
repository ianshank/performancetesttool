...
class CloudWatchIntegration:
        ...
        try:
            ...
            if credentials.get("access_key_id") and credentials.get("secret_access_key"):
                self.client = boto3.client(
                    ...
                    region_name=credentials.get("region", env_config.get("aws_region", "us-west-2"))
                )
            else:
                # Use default credentials (IAM role, profile, etc.)
                self.client = boto3.client(
                    'cloudwatch',
                    region_name=env_config.get("aws_region", "us-west-2")
                )
    ...
    def get_metrics(self, namespace: str = "AWS/EC2", 
        ...
        try:
            ...
            self.logger.info(f"Retrieved {len(response['Datapoints'])} data points for {metric_name}")
    ...
    def put_custom_metric(self, namespace: str, metric_name: str, 
