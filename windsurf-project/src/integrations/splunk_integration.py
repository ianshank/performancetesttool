...
class SplunkIntegration:
    ...
    def get_application_logs(self, application: str, index: str = "main", 
    ...
    def get_request_count_metrics(self, index: str = "main", hours: int = 1) -> List[Dict[str, Any]]:
        """Get request count metrics"""
        query = f"search index={index} sourcetype=access_combined"
        return self.get_metrics(query, hours=hours)
    
    def get_response_time_metrics(self, index: str = "main", hours: int = 1) -> List[Dict[str, Any]]:
        """Get response time metrics"""
        query = f"search index={index} sourcetype=access_combined | stats avg(response_time) by _time"
        return self.get_metrics(query, hours=hours)
    ...
    def create_saved_search(self, name: str, query: str, 
                           cron_schedule: Optional[str] = None) -> bool:
        """
        ...
        data = {
            "name": name,
            "search": query,
            "enableSched": "1" if cron_schedule else "0"
        }
